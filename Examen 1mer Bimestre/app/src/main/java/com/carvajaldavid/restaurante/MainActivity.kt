...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnUpdate.setOnClickListener{
            if(txtID.text.toString()==""|| txtNombre.text.toString()==""){

                Toast.makeText(this,
                    "Ingrese el id y un nombre", Toast.LENGTH_SHORT).show();

            }else if(!(txtID.text.toString().toInt() in 1..5)){
                Toast.makeText(this,
                    "Ingrese un ID existente: 1, 2, 3, 4, 5", Toast.LENGTH_SHORT).show();

            }else{
                val ingredientes= DataCompanion.actualizarIngredientes();
                val receta: Array<Receta?> = DataCompanion.actualizarReceta(txtID.text.toString().toInt(),txtNombre.text.toString());
                guardarData(ingredientes,receta);
                Toast.makeText(this,
                    "Se a actualizado correctamente", Toast.LENGTH_SHORT).show();
            }
